...
def _denoised_path(canon, sub, space, ses, task, bld):
    return (Path(canon) / 'mri_data' / sub / f'{space}111' / ses /
            f'{task}_{bld:03d}' / 'tedana' / f'{ses}_bld{bld:03d}_desc-denoised_bold.nii.gz')
...
def _active(sub):
    ...
    out = subprocess.run(['squeue', '-u', sub_user(), '-h', '-o', '%j'],
...
def _container_cmd(args, ses, task, bld, space):
    bld3 = f'{int(bld):03d}'
    outdir = (f'{args.canonical_root}/mri_data/{args.sub}/{space}111/{ses}/'
              f'{task}_{bld3}/tedana')
    ...
    return (f"apptainer exec --bind /oak:/oak,/scratch:/scratch {args.sif} "
            f"bash -c '{inner}'")
...
def _inflight_count(sub: str, space: str) -> int:
    ...
    out = subprocess.run(['squeue', '-u', sub_user(), '-h', '-o', '%j'],
                         capture_output=True, text=True).stdout
    return sum(1 for ln in out.split()
               if ln.startswith(JOBPREFIX + '_') and ln.endswith('_' + space)
...
def _submit_one(args, label, ses, task, bld, space, logdir: Path):
    ...
    sb = ['sbatch', '--parsable', '--job-name', name,
          '--partition', args.partition, '--time', args.time,
          '--mem', args.mem, '--cpus-per-task', str(args.cpus),
          '--output', str(logdir / f'slurm_teda_{space}_%j.log'),
          '--error', str(logdir / f'slurm_teda_{space}_%j.err'),
          '--wrap', _container_cmd(args, ses, task, bld, space)]
...
def cmd_list(args):
    ...
    log.info('%d scans x %d spaces = %d units: %d done, %d pending',
             len(units), len(SPACES), len(units) * len(SPACES), done, pending)
    return 0
...
''')
    ...
    n = len(pending)
    sb = ['sbatch', '--parsable', '--job-name', f'{JOBPREFIX}_array_{tag}',
          '--array', f'0-{n - 1}%{args.throttle}',
          '--partition', args.partition, '--time', args.time,
          '--mem', args.mem, '--cpus-per-task', str(args.cpus),
          '--output', str(scatter / 'logs' / f'tedana_array_{tag}_%A_%a.log'),
          '--error', str(scatter / 'logs' / f'tedana_array_{tag}_%A_%a.err'),
          '--wrap', f'bash {runner}']
    r = subprocess.run(sb, capture_output=True, text=True, check=True)
    log.info('submitted tedana array: %d pending units, throttle %%%d, job %s',
             n, args.throttle, r.stdout.strip())
    ...
    for label, ses, task, bld in units:
        ...
        for space in spaces:
            ...
            try:
                ...
                log.warning('sbatch rejected (%s); stopping this submit cycle.',
                            err[-1] if err else exc)
                log.info('tedana: submitted %d before cap, skipped %d (done) + %d (active).',
                         submitted, skip_done, skip_active)
                return 0
            submitted += 1
            if budget is not None:
                budget -= 1
            log.info('submitted %-34s mem=%s job %s', key, args.mem, jid)
    log.info('tedana: submitted %d, skipped %d (done) + %d (active).',
             submitted, skip_done, skip_active)
    return 0
...
def main(argv=None):
    ...
    p.add_argument('--canonical-root',
                   default='/scratch/users/logben/discovery_bids/derivatives/iproc')
    p.add_argument('--scatter-root',
    ...
    p.add_argument('--throttle', type=int, default=12,
                   help='max concurrent array tasks (submit-array mode)')
    ...
    return {'list': cmd_list, 'submit': cmd_submit,
            'submit-array': cmd_submit_array}[args.mode](args)
