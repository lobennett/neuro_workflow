...
def _bid_walk(bids_root: Path, task: str) -> Iterable[Path]:
    ...
    """
    return sorted(
        bids_root.rglob(f'sub-*/ses-*/func/sub-*_ses-*_task-{task}_run-*_events.tsv')
    )
...
def render_markdown(audits: list[TaskAudit]) -> str:
    ...
    lines += ['Subset matched 0 rows across every scan in a cohort.  The regressor',
        ...
        for reg, by_cohort in a.regressor_stats.items():
            for cohort, stats in by_cohort.items():
                if stats['n_scans'] > 0 and stats['total_matches'] == 0:
                    critical_found = True
                    lines.append(
                        f"| {a.task_name} | {reg} | {cohort} | {stats['n_scans']} | 0 |"
                    )
    ...
    lines += ['Regressors where the YAML hardcodes a numeric duration but the',
    ...
    lines += ['| Task | Regressor | Cohort | YAML duration | Observed range (min..max, median) | Total matches |',
              '|---|---|---|---|---|---|']
    ...
    lines += ['Regressor matches some but not all scans.  Often legitimate',
              '(e.g. `nogo_failure` rare in good performers) but flagged for review.', '']
    lines += ['| Task | Regressor | Cohort | Scans w/ zero | Total scans |',
              '|---|---|---|---|---|']
    minor_found = False
    for a in audits:
        for reg, by_cohort in a.regressor_stats.items():
            for cohort, stats in by_cohort.items():
                ...
                if 0 < z < stats['n_scans']:
                    minor_found = True
                    lines.append(
                        f"| {a.task_name} | {reg} | {cohort} | {z} | {stats['n_scans']} |"
                    )
    if not minor_found:
        lines.append('| _(none)_ | | | | |')
    lines.append('')
    ...
    lines += ['Trial_id values that appear in events.tsv but are not the literal',
    ...
    for a in audits:
        ...
        lines.append('')
        lines.append('| Regressor | Cohort | Scans | Total matches | YAML dur | Obs dur (min..max, med) |')
        lines.append('|---|---|---|---|---|---|')
        for reg, by_cohort in a.regressor_stats.items():
            for cohort, stats in by_cohort.items():
                if stats['duration_median'] is None:
                    ...
                    obs = (f"{stats['duration_min']:.2f}..{stats['duration_max']:.2f}, "
...
def main() -> int:
    ...
    parser.add_argument('--validation', type=Path,
                        default=Path('/scratch/users/logben/validation_bids'))
    parser.add_argument('--output', type=Path,
                        default=Path('docs/EVENTS-TASK-CONFIG-AUDIT.md'))
