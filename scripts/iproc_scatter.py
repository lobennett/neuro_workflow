...
SCHEMA_HEADER = ('SUBJID,SESSION_ID,Analyze,BLD,TYPE,ANAT,FMAP_MAG,FMAP_PHASE,'
...
def target_nvols(canonical_root, sub, bold_row):
    ...
    par = (Path(canonical_root) / 'mri_data' / sub / 'NAT' / sess / f'{task}_{bld}' /
...
def build_units(rows, midvol_sess, midvol_boldno):
    ...
    mv_bold = next(r for r in mv_rows if r['TYPE'] not in ('FMAP', 'ANAT')
                   and int(r['BLD']) == int(midvol_boldno))
    ...
    for sess in sorted(by_sess):
        ...
        bold_rows = [r for r in by_sess[sess]
                     if r['TYPE'] not in ('FMAP', 'ANAT') and r['Analyze'] == '1']
        for b in bold_rows:
            label = f"ses-{sess}_{b['TYPE']}_{int(b['BLD']):03d}"
            if label == mv_label:
                units.append({'label': label, 'session': sess, 'bold_rows': [b],
                              'fmap_rows': fmap_rows, 'is_wave1': True})
            else:
                ...
                if sess != midvol_sess:
                    ...
                    brows += [mv_bold]      # same (midvol) session -> add midvol bold; fmap already present
                units.append({'label': label, 'session': sess, 'bold_rows': brows,
                              'fmap_rows': frows, 'is_wave1': False})
    return units
...
def cmd_generate(args):
    ...
    write_scatter_cluster_requests(canon / 'configs' / 'cluster_requests.csv',
                                   scatter_cr, COMBINE_SKIP_STEPS)
    ...
    for u in units:
        ...
        manifest.append({'label': u['label'], 'session': u['session'],
                         'wave': 1 if u['is_wave1'] else 2,
                         'target_nvols': nvols if nvols is not None else '',
                         'cfg': str(cfg), 'scanlist': str(sl), 'logdir': str(jobdir)})
    mpath = scatter / 'units_manifest.tsv'
    with mpath.open('w', newline='') as fh:
        w = csv.DictWriter(fh, fieldnames=['label', 'session', 'wave', 'target_nvols',
                                           'cfg', 'scanlist', 'logdir'], delimiter='\t')
...
def _submit_one(unit, args, dependency=None):
    ...
    sb = ['sbatch', '--parsable', '--job-name', f'{pfx}_{unit["label"]}',
          '--partition', args.partition, '--time', args.time, '--mem', mem,
          '--cpus-per-task', str(args.cpus),
          '--output', str(jobdir / f'slurm_{args.stage[:4]}_%j.log'),
          '--error', str(jobdir / f'slurm_{args.stage[:4]}_%j.err')]
    if dependency:
...
def cmd_validate(args):
    ...
    m = next((u for u in units if u['label'] == args.scan
              or u['label'].endswith(args.scan.replace('/', '_'))), None)
    if not m:
        log.error('no unit matches %s', args.scan); return 2
    args._dry = True
...
def _midvol_done(args):
    ...
    if args.stage == 'combine_and_apply_warp':
        need = (list((base / 'NAT111' / args.midvol_sess).glob('FLANKER_*/*_mc_unwarp_anat_e?.nii.gz'))
...
def cmd_submit_rest(args):
    ...
    if not ok:
        log.error('[%s] midvol (FLANKER009) outputs not present (%d found); run the '
                  'wave-1 pilot for this stage first.', args.stage, len(found))
        return 2
    ...
    for u in units:
        ...
        submitted += 1
        log.info('submitted %-42s mem=%-5s job %s%s', u['label'], mem, jid,
                 f' (afterok combine:{dep})' if dep else '')
    log.info('[%s] submitted %d, skipped %d (active) + %d (already done)%s.',
             args.stage, submitted, skipped_active, skipped_done,
...
def main(argv=None):
    ...
    p.add_argument('--stage', default='combine_and_apply_warp',
                   choices=list(STAGE_CFG))
    p.add_argument('--sub', default='s10')
    p.add_argument('--canonical-root', default='/scratch/users/logben/discovery_bids/derivatives/iproc')
    p.add_argument('--scatter-root', default='/scratch/users/logben/discovery_bids/derivatives/iproc/scatter_combine_s10')
    ...
    return {'generate': cmd_generate, 'validate': cmd_validate,
            'submit-pilot': cmd_submit_pilot, 'submit-rest': cmd_submit_rest}[args.mode](args)
