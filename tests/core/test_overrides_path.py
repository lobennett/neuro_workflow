...
def test_committed_force_exclude_is_loaded_and_applied(tmp_path, monkeypatch):
    _isolate(tmp_path, monkeypatch)
    save_source_entries("testds", "motion", [{
        "subject": "sub-s1", "session": "ses-01", "task": "task-flanker",
        "run": "run-1", "action": "exclude", "source": "motion", "reason": "FD",
    }])
    (tmp_path / "lock" / "testds_overrides.json").write_text(json.dumps([{
        "subject": "sub-s2", "session": "ses-02", "task": "task-nBack",
        "run": "run-1", "action": "force-exclude", "reason": "manual",
    }]))
...
def test_force_exclude_is_idempotent_against_source_entries(tmp_path, monkeypatch):
    ...
    _isolate(tmp_path, monkeypatch)
    save_source_entries("testds", "qa_decisions", [{
        "subject": "sub-s1", "session": "ses-01", "task": "task-nBack",
        "run": "run-1", "action": "exclude", "source": "qa_decisions", "reason": "junk-trial",
    }])
    (tmp_path / "lock" / "testds_overrides.json").write_text(json.dumps([
        # duplicates the source-excluded scan -> must not double up
        {"subject": "sub-s1", "session": "ses-01", "task": "task-nBack",
         "run": "run-1", "action": "force-exclude", "reason": "Behavioral exclusion"},
        # a brand-new scan -> must be added
        {"subject": "sub-s2", "session": "ses-02", "task": "task-flanker",
         "run": "run-1", "action": "force-exclude", "reason": "Behavioral exclusion"},
    ]))
    compiled = compile_exclusions("testds")
    keys = [(e["subject"], e["session"], e["task"], e["run"]) for e in compiled]
    ...
    dup = [e for e in compiled if (e["subject"], e["session"], e["task"], e["run"]) == ("sub-s1", "ses-01", "task-nBack", "run-1")]
...
def test_committed_force_include_removes_a_compiled_entry(tmp_path, monkeypatch):
    ...
    (tmp_path / "lock" / "testds_overrides.json").write_text(json.dumps([{
        "subject": "sub-s1", "session": "ses-01", "task": "task-flanker",
        "run": "run-1", "action": "force-include", "reason": "rescued",
    }]))
    compiled = compile_exclusions("testds")
    keys = {(e["subject"], e["session"], e["task"], e["run"]) for e in compiled}
