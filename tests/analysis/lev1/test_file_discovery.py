...
class TestFileFinderIntegration:
    ...
    def test_get_files_incomplete_structure(self, temp_dir):
        ...
        (
            subj_dir / 'sub-s001_ses-01_task-rest_run-01_desc-confounds_timeseries.tsv'
        ).write_text('confounds')
    ...
    def test_get_files_with_custom_required_files(self, temp_dir):
        ...
        files = finder.get_files(
            'sub-s001', 'rest', required_files=['events', 'confounds']
        )
...
class TestSurfacePatterns:
    ...
    def test_get_files_fsaverage6_patterns(self, temp_dir):
        ...
        # Confounds
        (subj_dir_fmriprep / 'sub-s001_ses-01_task-rest_run-01_desc-confounds_timeseries.tsv').write_text('mock')
        # fsaverage6 surface files
        (subj_dir_fmriprep / 'sub-s001_ses-01_task-rest_run-01_hemi-L_space-fsaverage6_bold.func.gii').write_text('mock')
        (subj_dir_fmriprep / 'sub-s001_ses-01_task-rest_run-01_hemi-R_space-fsaverage6_bold.func.gii').write_text('mock')

        finder = FileFinder(bids_dir, fmriprep_dir)
        required = FileFinder.get_required_files_for_space('fsaverage6')
        files = finder.get_files('s001', 'rest', required_files=required, surface_space='fsaverage6')
    ...
    def test_get_files_fsnative_is_default(self, temp_dir):
        ...
        (subj_dir_fmriprep / 'sub-s001_ses-01_task-rest_run-01_desc-confounds_timeseries.tsv').write_text('mock')
        (subj_dir_fmriprep / 'sub-s001_ses-01_task-rest_run-01_hemi-L_space-fsnative_bold.func.gii').write_text('mock')
        (subj_dir_fmriprep / 'sub-s001_ses-01_task-rest_run-01_hemi-R_space-fsnative_bold.func.gii').write_text('mock')
...
def test_filefinder_respects_mni_template_and_res(tmp_path):
    ...
    files = finder_default.get_files(
        subject_id='sub-s01', task_name='rest',
    ...
    finder_override = FileFinder(
        bids_dir=bids, fmriprep_dir=fmriprep,
        mni_template='MNI152NLin2009cAsym', mni_res='1',
    )
