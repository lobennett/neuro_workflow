...
def test_compute_motion_returns_dataclass(tmp_path):
    path = _make_confounds(tmp_path, n_vols=5, fd=[0.1, 0.2, 0.3, 0.4], dvars=[1.0, 1.1, 1.2, 1.3])
...
def test_compute_motion_proportion_over_thresholds(tmp_path):
    ...
    # → 2/4 = 50% over 0.5
    path = _make_confounds(tmp_path, n_vols=5, fd=[0.1, 0.6, 0.7, 0.05], dvars=[1.0, 1.6, 1.7, 1.0])
    m = compute_motion(path)
