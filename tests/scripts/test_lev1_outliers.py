...
def _write_compiled_exclusions(path):
    ...
    path.write_text(json.dumps([
        {
            "subject": "sub-s10", "session": "ses-05",
            "task": "task-goNogo", "run": "run-1",
            "source": "qa_decisions", "action": "exclude", "reason": "x",
        },
    ]))


def _run_main(argv):
    with patch("scripts.lev1_outliers.detect_lev1_outliers") as mock_detect, \
...
def test_main_missing_exclusions_file_errors_loud(tmp_path):
    ...
    rc, mock_detect = _run_main([
        ...
        "--lev1-dir", str(lev1),
        "--output-dir", str(tmp_path / "out"),
        "--exclusions-file", str(tmp_path / "does_not_exist.json"),
    ])
    assert rc == 1
