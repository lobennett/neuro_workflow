...
def _load_module():
    ...
    script_path = (
        Path(__file__).resolve().parents[2] / "scripts" / "audit_subject_flywheel_vs_bids.py"
    )
...
def test_audit_marks_excluded_per_config(tmp_path):
    ...
    fw_sessions = [
        {
            "fw_session_label": "20210101",
            "timestamp": "2021-01-01T10:00:00",
            "acquisitions": [{"label": "T1w_MPRAGE"}, {"label": "BOLD_rest"}],
        },
        {
            "fw_session_label": "25210",
            "timestamp": "2022-05-24T17:10:00",
...
def test_audit_marks_reassigned_per_config(tmp_path):
    ...
    fw_sessions = [
        {
            "fw_session_label": "22752",
            "timestamp": "2021-02-12T09:00:00",
...
def test_render_audit_md_produces_table():
    ...
    rows = [
        mod.SessionAuditRow(
            fw_session_label="25210",
            fw_timestamp="2022-05-24T17:10:00",
            bids_session="EXCLUDED",
            n_t1w=1,
            n_t2w=0,
            n_bold=0,
            n_fmap=0,
