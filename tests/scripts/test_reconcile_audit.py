...
def test_view_membership_diff_flags_now_excluded_and_newly_included():
    ...
    view_scans = {
        ...
        "sub-s10_ses-01_task-cuedTS_run-1",
    }  # old view
    keep_scans = {
        "sub-s10_ses-02_task-shapeMatching_run-1",
        "sub-s10_ses-05_task-goNogo_run-1",
    }  # new keep-set
...
def test_scankey_from_name_parses_bold_and_confounds():
    ...
    assert (
        scankey_from_name("sub-s10_ses-02_task-shapeMatching_run-1_echo-2_bold.nii.gz")
        == "sub-s10_ses-02_task-shapeMatching_run-1"
    )
    assert (
        scankey_from_name("sub-s10_ses-02_task-shapeMatching_run-1_desc-confounds_timeseries.tsv")
        == "sub-s10_ses-02_task-shapeMatching_run-1"
    )
